...
const Dog = ({ idDog, item }) => {
  ...
  return (
    <div className=" w-full border border-gray-400  rounded-xl mb-2 bg-[#e6d7e7] shadow-md shadow-gray-400 ">
      <div className="w-full grid grid-cols-4 sm:justify-start sm:grid-cols-5 p-2 h-full items-center ">
        <div className="shrink-0">
          ...
          <p className="w-3/4 md:w-full ">{item.ownersName}</p>
          <p className="hidden sm:flex">{item.breed}</p>
        <div className="flex justify-between items-center">
          <div className="hidden lg:flex mr-1">{item.phone}</div>
